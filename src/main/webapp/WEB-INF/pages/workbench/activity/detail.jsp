...
<script type="text/javascript">
	...
	$(function(){
		...
		// });
		//因为上面那个函数只能绑定固有元素的事件，而我们新创建的一个备注是动态元素，不能绑定这个事件，故要改成下面这个
	...
	//	给保存按钮添加单机事件，向后台提交市场活动备注
		$("#saveCreateACtivityRemarkBtn").click(function () {
			var noteContent=$.trim($("#remark").val());
			//下面这条语句是用EL表达式从作用域中获得信息，在这里必须带引号
			...
			$.ajax({
				...
				success:function (data){
					if(data.code=="1"){
						...
						htmlStr+="<h5>"+data.noteContent+"</h5>";
	...
	//给所有的删除图标添加单击事件,用户无论点哪一个图标，都会执行我们定义的函数,有因为存在动态标签，所以使用on标签
		$("#remarkDivList").on("click","a[name='deleteA']",function () {
			//收集参数
			//这个的this代指的是dom对象，我们想要获得jquery对象需要加$，想要获得属性值，用attr函数
